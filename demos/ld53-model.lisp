	 ...
	 (when player-stats
		(let ((z (clist-get player-stats 'z))
		  ...
		  (let ((v-x 0.0)
				  ...
				  (v-z 0.0)
				  (v-angle 0.0))
			 (when (key-down? foxgl:key-up foxgl:key-w)
				(set! v-z 0.1))
			 (when (key-down?  foxgl:key-left foxgl:key-a)
				(set! v-x 0.1))
			 (when (key-down? foxgl:key-d foxgl:key-right)
				(set! v-x -0.1))
			 (when (key-down? foxgl:key-s foxgl:key-down)
				(set! v-z -0.1))
