...
(defvar house-1
  ...
  (offset (1 1.5 -2)
    (rgb (0.7 0.7 1)
      (scale (0.7 0.7 0.1)
        (cube))))))
(defun lisp:print-scope-all (scope)
  (let ((i 0))
	 (loop scope 
			 (println (cons 'scope i))
			 (lisp:print-scope scope)
			 (set! scope (lisp:scope-super scope))
			 (set! i (1+ i))
			 ))
  )
...
(defvar evil-wizard-model
  '(vars ((wizard-model-color '(0.75 0.25 0.2)))
	 (wizard-model)))
(defvar evil-wizard-model2
  '(vars ((wizard-model-color '(0.1 0.1 0.1)))
	 (wizard-model)))
