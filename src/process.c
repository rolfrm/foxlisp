...
static lisp_value lisp_fork(){
  return integer(fork());
}

static lisp_value lisp_exec(lisp_value filename, lisp_value argv, lisp_value env){
...
static lisp_value posix_read(lisp_value fd, lisp_value buffer){
  int rd = read(fd.integer, buffer.vector->data, vector_length(buffer).integer * buffer.vector->elem_size);
  return integer(rd);
}

static lisp_value posix_write(lisp_value fd, lisp_value buffer){
  int rd = write(fd.integer, buffer.vector->data, vector_length(buffer).integer * buffer.vector->elem_size);
  return integer(rd);
}
