...
inline bool is_cons(lisp_value v) { return v.type == LISP_CONS; }
...
inline bool is_float32(lisp_value a) { return a.type == LISP_FLOAT32; }

inline bool is_string(lisp_value a) { return a.type == LISP_STRING; }

inline bool is_symbol(lisp_value a) { return a.type == LISP_SYMBOL; }

inline bool is_regular_symbol(lisp_value a) {
  return is_symbol(a) && !is_keyword(a);
}

inline bool is_function(lisp_value a) { return a.type == LISP_FUNCTION; }
...
bool is_scope(lisp_value a) { return a.type == LISP_SCOPE; }

inline bool is_keyword(lisp_value sym) {
  return is_symbol(sym) && lisp_value_symbol(sym) >= 0x100000;
}

inline f64 lisp_value_rational(lisp_value v) { return v.rational; }
...
inline char *lisp_value_string(lisp_value v) { return v.string; }

inline lisp_value integer_lisp_value(i64 i) {
  return (lisp_value){.type = LISP_INTEGER, .integer = i};
}

inline int64_t lisp_value_integer(lisp_value v) { return v.integer; }
...
inline u8 lisp_value_byte(lisp_value v) { return (u8)v.integer; }

inline void *lisp_value_pointer(lisp_value val) { return val.pointer; }
...
inline lisp_symbol lisp_value_symbol(lisp_value val) { return val.symbol; }
...
inline cons *lisp_value_cons(lisp_value val) { return val.cons; }
...
lisp_scope *lisp_value_scope(lisp_value val) { return val.pointer; }

inline bool lisp_value_eq(lisp_value a, lisp_value b) {
  if (lisp_value_type(a) != lisp_value_type(b))
    return false;
  return a.integer == b.integer;
}

inline lisp_value bool_lisp_value(bool v) { return v ? t : nil; }

inline lisp_value global_index_lisp_value(size_t i) {
  return (lisp_value){.type = LISP_GLOBAL_INDEX, .integer = i};
}

inline lisp_value local_index_lisp_value(size_t scope_level, size_t scope_index,
                                         bool scope_type) {
