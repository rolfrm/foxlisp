...
lisp_value lisp_read_fd(lisp_value item, lisp_value buffer) {
  ...
  int r = read(fd, buffer.vector->data,
               buffer.vector->count *
                   lisp_type_size(buffer.vector->default_value.type));
  if (r == -1)
    return nil;
  return integer(r);
}
...
lisp_value lisp_recv_fd(lisp_value item, lisp_value buffer) {
  ...
  if (stat == 0)
    return integer(0);
  int r = recv(fd, buffer.vector->data,
               buffer.vector->count *
                   lisp_type_size(buffer.vector->default_value.type),
               0);
...
lisp_value lisp_write_fd(lisp_value item, lisp_value buffer) {
  ...
  ssize_t written = write(
      fd, buffer.vector->data,
      buffer.vector->count * lisp_type_size(buffer.vector->default_value.type));
