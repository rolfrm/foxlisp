...
lisp_value tokenize_stream(io_reader *rd) {
  ...
  if (c == '`') {
    io_read_u8(rd);
    return new_cons_r(rd, quasiquote_sym,
                      new_cons_r(rd, tokenize_stream(rd), nil));
  }
  if (c == ',') {
    io_read_u8(rd);
    if (io_peek_u8(rd) == '@') {
      io_read_u8(rd);
      return new_cons_r(rd, unquote_splice_sym,
                        new_cons_r(rd, tokenize_stream(rd), nil));
    }
    return new_cons_r(rd, unquote_sym,
                      new_cons_r(rd, tokenize_stream(rd), nil));
  }
  if (c == '(') {
    io_read_u8(rd);
...
lisp_value lisp_read_stream(io_reader *rd) { return tokenize_stream(rd); }
